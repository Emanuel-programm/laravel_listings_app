...
<header class="bg-blue-900 text-white p-4" x-data="{open:false}">
    <div class="container mx-auto flex justify-between items-center">
      ...
      <nav class="hidden md:flex items-center space-x-4">
        ...
        <x-nav-link url="/bookmarks" :active="request()->is('/bookmarks')">Jobs saved </x-nav-link>
...
<a href="{{ route('dashboard') }}">
  @if (Auth::user()->avatar)
    <img src="{{ asset('storage/'. Auth::user()->avatar) }}" alt="{{ Auth::user()->name }}" class="w-1- h-10 rounded-full">
  @else
    <img src="{{ asset('storage/avatars/default_avatar.png') }}" alt="{{ Auth::user()->name }}" class="w-1- h-10 rounded-full">

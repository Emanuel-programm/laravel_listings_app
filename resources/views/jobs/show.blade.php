    <x-layout>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div class="rounded-lg shadow-md bg-white p-3">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex space-x-3 ml-4">
                            ...
                            >
                            <!-- Delete Form -->
                            <form method="POST" action="{{ route('jobs.destroy',$job->id)}}" onsubmit="return confirm('Are you sure that you want delete this job?')">
                                @csrf
            ...
            <div  x-show="open"  class="fixed inset-0 flex items-center p-4 justify-center bg-gray-500 bg-opacity-50">
               <div @click.away="open=false" class="bg-white p-6 max-h-[100vh] overflow-y-auto  rounded-lg shadow-md w-full max-w-md">
                <h3 class="text-lg font-semibold mb-4">
                    Apply for {{ $job->title }}
                </h3>
                <form action="" enctype="multipart/form-data">
                @csrf
                <x-inputs.text id="full_name" :required="true" name="full_name" label="Full name"/>
                <x-inputs.text id="contact_phone" :required="true" name="contact_phone" label="Contact phone"/>
                <x-inputs.text id="contact_email" :required="true" name="contact_email" label="Contact Email"/>
                <x-inputs.text-area id="message"  name="message" label="Email"/>
        ...
        Cancel
               ...
               @else
                  <p class="my-5 bg-gray-200 rounded-xl  p-3">
                   <i class="fa fa-info-circle mr-3"></i> You must be logged in to apply for this job.
                    </p>
                  @endauth
                </div>
            ...
            <p class="mt-10 bg-gray-200 text-gray-700 font-bold w-full py-2 px-4 rounded-full text-center">
                <i class="fas fa-info-circle mr-3"></i> You need to login to bookmark this job
            </p>
            ...
            @method('DELETE')
          <button type="submit"  class=" bg-red-500 text-white font-bold w-full py-2 px-4 rounded-full flex items-center justify-center hover:bg-red-600">
    <i class="fas fa-bookmark mr-3"></i>Remove Bookmark
            @else
            <button type="submit"  class=" bg-blue-500 text-white font-bold w-full py-2 px-4 rounded-full flex items-center justify-center hover:bg-blue-600">
